...
from pyvenafi.tpp.api.websdk.enums.config import ClientWorkAttributeValues
from pyvenafi.tpp.attributes.certificate_provisioning_work import CertificateProvisioningWorkAttributes
from pyvenafi.tpp.attributes.client_agent_automatic_upgrade_work import ClientAgentAutomaticUpgradeWorkAttributes
from pyvenafi.tpp.attributes.client_agent_configuration_work import ClientAgentConfigurationWorkAttributes
from pyvenafi.tpp.attributes.client_agent_ssh_discovery_work import ClientAgentSSHDiscoveryWorkAttributes
from pyvenafi.tpp.attributes.client_agent_ssh_key_usage_work import ClientAgentSSHKeyUsageWorkAttributes
from pyvenafi.tpp.attributes.client_agent_ssh_provisioning_work import ClientAgentSSHProvisioningWorkAttributes
from pyvenafi.tpp.attributes.client_certificate_discovery_work import ClientCertificateDiscoveryWorkAttributes
from pyvenafi.tpp.attributes.client_certificate_work import ClientCertificateWorkAttributes
from pyvenafi.tpp.attributes.client_user_certificate_work import ClientUserCertificateWorkAttributes
from pyvenafi.tpp.attributes.client_work_base import ClientWorkBaseAttributes
from pyvenafi.tpp.attributes.network_device_certificate_work import NetworkDeviceCertificateWorkAttributes
from pyvenafi.tpp.attributes.server_agent_cert_device_placement_work import ServerAgentCertDevicePlacementWorkAttributes
from pyvenafi.tpp.attributes.server_agent_ssh_device_placement_work import ServerAgentSSHDevicePlacementWorkAttributes
from pyvenafi.tpp.attributes.server_certificate_work import ServerCertificateWorkAttributes
...
class AgentConnectivity(_ClientWorkBase):
    ...
    def schedule(
        self, work: 'Union[config.Object, str]', start_time: int = None, daily: bool = False, hourly: bool = False,
        ...
        if daily:
            ...
            attributes[
                ClientAgentConfigurationWorkAttributes.schedule_type] = ClientWorkAttributeValues.AgentConnectivity.ScheduleType.daily
        elif hourly:
            attributes[
                ClientAgentConfigurationWorkAttributes.schedule_type] = ClientWorkAttributeValues.AgentConnectivity.ScheduleType.hourly
        elif days_of_week:
            if not start_time:
                raise InvalidFormat(
            ...
            attributes[
                ClientAgentConfigurationWorkAttributes.schedule_type] = ClientWorkAttributeValues.AgentConnectivity.ScheduleType.days_of_week
            ...
            attributes[
                ClientAgentConfigurationWorkAttributes.schedule_type] = ClientWorkAttributeValues.AgentConnectivity.ScheduleType.days_of_month
    ...
    def create(
        ...
        server_url: str = "",
        proxy_url: str = "",
        proxy_credentials: 'Union[config.Object, str]' = None,
...
class CertificateDevicePlacement(_ClientWorkBase):
    ...
    def create(
        self, name: str, placement_folder: 'Union[config.Object, str]', share_mode: int = 2,
...
class CertificateDiscovery(_ClientWorkBase):
    ...
    def create(
        self,
        name: str,
        certificate_location: 'Union[config.Object, str]',
...
class CertificateEnrollmentViaESTProtocol(_ClientWorkBase):
    ...
    def create(
        self,
        name: str,
        certificate_container: 'Union[config.Object, str]',
        naming_pattern: str,
        ca_template: 'Union[config.Object, str]',
        contacts: 'list[ident.Identity, str]',
        certificate_origin: str = None,
        ...
        authentication_credentials: 'Union[config.Object, str]' = None,
        authenticate_only_by_password: bool = False,
        ...
        trusted_certs_and_cas: 'list[config.Object, str]' = None,
        get_if_already_exists: bool = False,
        attributes: dict = None
    ):
        """
        Args:
        ...
        work_attributes = {
            ...
            NetworkDeviceCertificateWorkAttributes.fallback_to_http_auth                      : int(
                authenticate_only_by_password is False
            ),
            NetworkDeviceCertificateWorkAttributes.revoke_existing_certificate_on_reenrollment: int(
                revoke_previous_version
            ),
            NetworkDeviceCertificateWorkAttributes.pop_mode                                   : identity_verification
        }

        if certificate_origin:
            work_attributes[NetworkDeviceCertificateWorkAttributes.origin] = certificate_origin
        if certificate_description:
            work_attributes[NetworkDeviceCertificateWorkAttributes.description] = certificate_description
        if authentication_credentials:
            work_attributes[NetworkDeviceCertificateWorkAttributes.authentication_credentials] = self._get_dn(
...
class CertificateInstallation(_ClientWorkBase):
    ...
    def schedule(
        ...
        if len(
            [x for x in [daily, hourly, on_receipt, days_of_week, days_of_month, every_x_minutes] if
             x not in [None, False]]
        ) != 1:
            raise InvalidFormat(
        ...
        if daily:
            ...
            attributes[
                ClientCertificateDiscoveryWorkAttributes.schedule_type] = ClientWorkAttributeValues.CertificateInstallation.ScheduleType.days_of_month
            attributes[ClientCertificateDiscoveryWorkAttributes.days_of_month] = days_of_month
        elif on_receipt:
            attributes[
                ClientCertificateDiscoveryWorkAttributes.schedule_type] = ClientWorkAttributeValues.CertificateInstallation.ScheduleType.on_receipt
        elif every_x_minutes:
            if every_x_minutes == 30:
                attributes[
                    ClientCertificateDiscoveryWorkAttributes.schedule_type] = ClientWorkAttributeValues.CertificateInstallation.ScheduleType.every_x_minutes
    ...
    def create(
        self, name: str, log_threshold: str = ClientWorkAttributeValues.CertificateInstallation.LogThreshold.info,
        attributes: dict = None, get_if_already_exists: bool = True
    ):
        """
        Args:
...
class DeviceCertificateCreation(_ClientWorkBase):
    ...
    def create(
        self, name: str, certificate_container: 'Union[config.Object, str]', ca_template: 'Union[config.Object, str]',
        ...
        if description:
            work_attributes[ClientCertificateWorkAttributes.description] = description
        if organization:
            work_attributes[ClientCertificateWorkAttributes.organization] = organization
        if city_locality:
            work_attributes[ClientCertificateWorkAttributes.city] = city_locality
        if state_province:
            work_attributes[ClientCertificateWorkAttributes.state] = state_province
        if organizational_unit:
            work_attributes[ClientCertificateWorkAttributes.organizational_unit] = organizational_unit
        if country:
            work_attributes[ClientCertificateWorkAttributes.country] = country
        if subject_alternative_names:
            work_attributes[ClientCertificateWorkAttributes.x509_subjectaltname_dns] = common_name
        if automatic_renewal:
            work_attributes[ClientCertificateWorkAttributes.renewal_window] = renewal_days_before
...
class DynamicProvisioning(_ClientWorkBase):
    ...
    def create(
        ...
        naming_pattern: str = "$Client.DNSName$",
        common_name: str = "$Client.DNSName$",
        organization: str = None,
        organizational_unit: list[str] = None,
        city_locality: str = None,
        state_province: str = None,
        country: str = None,
        subject_alternative_names: str = "$Client.DNSname$",
        capi_keystore: bool = False,
        capi_friendly_name: str = "",
        capi_trustee: str = "",
        key_bit_strength: int = 2048,
        ...
        work_attributes = {
            ...
            ServerCertificateWorkAttributes.contact                : [self._get_prefixed_universal(c) for c in
                                                                      contacts] if contacts else None,
        ...
        if description:
            work_attributes[ServerCertificateWorkAttributes.description] = description
        if organization:
            work_attributes[ServerCertificateWorkAttributes.organization] = organization
        if city_locality:
            work_attributes[ServerCertificateWorkAttributes.city] = city_locality
        if state_province:
            work_attributes[ServerCertificateWorkAttributes.state] = state_province
        if organizational_unit:
            work_attributes[ServerCertificateWorkAttributes.organizational_unit] = organizational_unit
        if country:
            work_attributes[ServerCertificateWorkAttributes.country] = country
        if capi_keystore:
            work_attributes[
                ServerCertificateWorkAttributes.application_type] = ClientWorkAttributeValues.DynamicProvisioning.ApplicationType.capi
...
class SSHDevicePlacement(_ClientWorkBase):
    ...
    def create(
        self, name: str, devices_folder: 'Union[config.Object, str]',
...
class SSHDiscovery(_ClientWorkBase):
    ...
    def schedule(
        ...
        if len(
            [x for x in [daily, hourly, on_receipt, every_30_minutes, days_of_week, days_of_month] if
             x not in [None, False]]
        ) != 1:
            raise InvalidFormat(
    ...
    def create(
        self, name: str, scan_default_paths: bool = True, host_key_paths: list[str] = None,
        ...
        if len(scanner_paths) > 0:
            work_attributes[ClientAgentSSHDiscoveryWorkAttributes.ssh_scanner_service_path]: scanner_paths
        if len(user_paths) > 0:
            work_attributes[ClientAgentSSHDiscoveryWorkAttributes.ssh_scanner_user_path]: user_paths
...
class SSHKeyUsage(_ClientWorkBase):
    ...
    def create(
        self, name: str, limit_cache_size: int = 50000,
...
class SSHRemediation(_ClientWorkBase):
    ...
    def create(
        self, name: str, log_threshold: str = ClientWorkAttributeValues.SSHRemediation.LogThreshold.info,
        attributes: dict = None, get_if_already_exists: bool = True
    ):
        """
        Args:
...
class UserCertificateCreation(_ClientWorkBase):
    ...
    def create(
        ...
        work_attributes = {
            ...
            ClientUserCertificateWorkAttributes.certificate_authority            : ca_template_dn,
            ClientUserCertificateWorkAttributes.naming_pattern                   : naming_pattern,
            ClientUserCertificateWorkAttributes.contact                          : [self._get_prefixed_universal(c) for
                                                                                    c in
                                                                                    contacts] if contacts else None,
            ClientUserCertificateWorkAttributes.key_bit_strength                 : key_bit_strength,
            ClientUserCertificateWorkAttributes.x509_subject                     : common_name,
        ...
        if description:
            work_attributes[ClientUserCertificateWorkAttributes.description] = description
        if user_email:
        ...
        if automatic_renewal:
            work_attributes[ClientUserCertificateWorkAttributes.renewal_window] = renewal_days_before
        ...
        if configure_outlook:
            ...
            option_value = 0
            if outlook_encrypt_messages:
                option_value += 1
            if outlook_sign_outgoing:
                option_value += 2
            if not outlook_send_cleartext_signed:
                option_value += 32
            if outlook_request_receipts:
                option_value += 512
