...
from pyvenafi.tpp.api.websdk.models import ssh
...
class _SSH(WebSdkEndpoint):
    def __init__(self, api_obj):
        ...
        self.AddSelfServiceAuthorizedKey = self._AddSelfServiceAuthorizedKey(
            api_obj=self._api_obj,
            url=f'{self._url}/AddSelfServiceAuthorizedKey'
        )
        self.AddSelfServicePrivateKey = self._AddSelfServicePrivateKey(
            api_obj=self._api_obj,
            url=f'{self._url}/AddSelfServicePrivateKey'
        )
        self.AddUserPrivateKey = self._AddUserPrivateKey(api_obj=self._api_obj, url=f'{self._url}/AddUserPrivateKey')
        self.ApproveKeyOperation = self._ApproveKeyOperation(
            api_obj=self._api_obj,
            url=f'{self._url}/ApproveKeyOperation'
        )
        ...
        self.ChangePrivateKeyPassphrase = self._ChangePrivateKeyPassphrase(
            api_obj=self._api_obj,
            url=f'{self._url}/ChangePrivateKeyPassphrase'
        )
        self.ConfirmSelfServiceKeyInstallation = self._ConfirmSelfServiceKeyInstallation(
            api_obj=self._api_obj,
            url=f'{self._url}/ConfirmSelfServiceKeyInstallation'
        )
        self.DeleteUnmatchedKeyset = self._DeleteUnmatchedKeyset(
            api_obj=self._api_obj,
            url=f'{self._url}/DeleteUnmatchedKeyset'
        )
        ...
        self.EditSelfServiceAuthorizedKey = self._EditSelfServiceAuthorizedKey(
            api_obj=self._api_obj,
            url=f'{self._url}/EditSelfServiceAuthorizedKey'
        )
        self.ExportSelfServiceAuthorizedKey = self._ExportSelfServiceAuthorizedKey(
            api_obj=self._api_obj,
            url=f'{self._url}/ExportSelfServiceAuthorizedKey'
        )
        self.ExportSelfServicePrivateKey = self._ExportSelfServicePrivateKey(
            api_obj=self._api_obj,
            url=f'{self._url}/ExportSelfServicePrivateKey'
        )
        self.ImportAuthorizedKey = self._ImportAuthorizedKey(
            api_obj=self._api_obj,
            url=f'{self._url}/ImportAuthorizedKey'
        )
        ...
        self.MoveKeysetsToPolicy = self._MoveKeysetsToPolicy(
            api_obj=self._api_obj,
            url=f'{self._url}/MoveKeysetsToPolicy'
        )
        ...
        self.RollbackNonBlocking = self._RollbackNonBlocking(
            api_obj=self._api_obj,
            url=f'{self._url}/RollbackNonBlocking'
        )
        self.RotateNonBlocking = self._RotateNonBlocking(api_obj=self._api_obj, url=f'{self._url}/RotateNonBlocking')
        self.Rotate = self._Rotate(api_obj=self._api_obj, url=f'{self._url}/Rotate')
        self.SetUnmatchedKeysetPassPhrase = self._SetUnmatchedKeysetPassPhrase(
            api_obj=self._api_obj,
            url=f'{self._url}/SetUnmatchedKeysetPassPhrase'
        )
        self.SkipKeyRotation = self._SkipKeyRotation(api_obj=self._api_obj, url=f'{self._url}/SkipKeyRotation')
        self.TestDeviceConnection = self._TestDeviceConnection(
            api_obj=self._api_obj,
            url=f'{self._url}/TestDeviceConnection'
        )
    ...
    class _AddAuthorizedKey(WebSdkEndpoint):
        ...
        def post(
            ...
            keyset_id: str,
            username: str,
            allowed_source_restriction: list[str] = None,
            denied_source_restriction: list[str] = None,
    ...
    class _AddUserPrivateKey(WebSdkEndpoint):
        # noinspection ALL
        def post(
            self, device_guid: str, filepath: str, username: str, format: str = None, keyset_id: str = None,
    ...
    class _EditSelfServiceAuthorizedKey(WebSdkEndpoint):
        def post(
            self, key_id: int, allowed_source_restriction: list[str] = None,
    ...
    class _ImportPrivateKey(WebSdkEndpoint):
        # noinspection ALL
        def post(
            self, device_guid: str, filepath: str, format: str, key_content_base_64: str, username: str,
            passphrase: str = None
        ):
            body = {
                'DeviceGuid'      : device_guid,
    ...
    class _KeysetDetails(WebSdkEndpoint):
        ...
        def post(
            self, page_size: int, keyset_filter: list[ssh.KeySetFilter] = None, load_key_data: bool = None,
            offset: int = None
        ):
            body = {
    ...
    class _RollbackNonBlocking(WebSdkEndpoint):
        def post(self, keyset_id: str):
            body = {
                'KeysetId': keyset_id
            }
    ...
    class _RotateNonBlocking(WebSdkEndpoint):
        def post(self, keyset_id: str, options: int = None, allow_skip_on_rotation: bool = None):
            body = {
                'KeysetId'           : keyset_id,
                'AllowSkipOnRotation': allow_skip_on_rotation,
        ...
        class _Stats(WebSdkEndpoint):
            def get(self, group_by: str):
                ...
                return generate_output(
                    output_cls=Output, response=self._get(params=params),
                    root_field='stats'
                )
