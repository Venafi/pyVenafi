class _Application:
    ...
    def amazon_aws(self):
        if not self._amazon_aws:
            from venafi.tpp.attributes.amazon_app import AmazonAppAttributes
    ...
    def azure_key_vault(self):
        if not self._azure_key_vault:
            from venafi.tpp.attributes.azure_key_vault import AzureKeyVaultAttributes
    ...
    def basic(self):
        if not self._basic:
            from venafi.tpp.attributes.basic import BasicAttributes
    ...
    def blue_coat_sslva(self):
        if not self._blue_coat_sslva:
            from venafi.tpp.attributes.bluecoat_sslva import BlueCoatSSLVAAttributes
    ...
    def capi(self):
        if not self._capi:
            from venafi.tpp.attributes.capi import CAPIAttributes
    ...
    def citrix_net_scaler(self):
        if not self._citrix_net_scaler:
            from venafi.tpp.attributes.netscaler import NetScalerAttributes
    ...
    def connect_direct(self):
        if not self._connect_direct:
            from venafi.tpp.attributes.connectdirect import ConnectDirectAttributes
    ...
    def f5_authentication_bundle(self):
        if not self._f5_authentication_bundle:
            from venafi.tpp.attributes.f5_authentication_bundle import F5AuthenticationBundleAttributes
    ...
    def f5_ltm_advanced(self):
        if not self._f5_ltm_advanced:
            from venafi.tpp.attributes.f5_ltm_advanced import F5LTMAdvancedAttributes
    ...
    def google_cloud_load_balancer(self):
        if not self._google_cloud_load_balancer:
            from venafi.tpp.attributes.google_cloud_app import GoogleCloudAppAttributes
    ...
    def ibm_datapower(self):
        if not self._ibm_datapower:
            from venafi.tpp.attributes.datapower import DataPowerAttributes
    ...
    def ibm_gsk(self):
        if not self._ibm_gsk:
            from venafi.tpp.attributes.gsk import GSKAttributes
    ...
    def imperva_mx(self):
        if not self._imperva_mx:
            from venafi.tpp.attributes.imperva_mx import ImpervaMXAttributes
    ...
    def jks(self):
        if not self._jks:
            from venafi.tpp.attributes.jks import JKSAttributes
    ...
    def juniper_sas(self):
        if not self._juniper_sas:
            from venafi.tpp.attributes.juniper_sas import JuniperSASAttributes
    ...
    def oracle_iplanet(self):
        if not self._oracle_iplanet:
            from venafi.tpp.attributes.iplanet import iPlanetAttributes
    ...
    def palo_alto_network_fw(self):
        if not self._palo_alto_network_fw:
            from venafi.tpp.attributes.palo_alto_network_fw import PaloAltoNetworkFWAttributes
    ...
    def pem(self):
        if not self._pem:
            from venafi.tpp.attributes.pem import PEMAttributes
    ...
    def pkcs11(self):
        if not self._pkcs11:
            from venafi.tpp.attributes.pkcs11 import PKCS11Attributes
    ...
    def pkcs12(self):
        if not self._pkcs12:
            from venafi.tpp.attributes.pkcs_12 import PKCS12Attributes
    ...
    def riverbed_steel_head(self):
        if not self._riverbed_steel_head:
            from venafi.tpp.attributes.riverbed_steelhead import RiverbedSteelHeadAttributes
    ...
    def tealeaf_pca(self):
        if not self._tealeaf_pca:
            from venafi.tpp.attributes.tealeaf_pca import TealeafPCAAttributes
    ...
    def vamnshield(self):
        if not self._vamnshield:
            from venafi.tpp.attributes.vam_nshield import VAMnShieldAttributes
...
class _ApplicationGroup:
    ...
    def apache(self):
        if not self._apache:
            from venafi.tpp.attributes.apache_application_group import ApacheApplicationGroupAttributes
    ...
    def pkcs11(self):
        if not self._pkcs11:
            from venafi.tpp.attributes.pkcs11_application_group import PKCS11ApplicationGroupAttributes
...
class _CertificateAuthority:
    ...
    def adaptable(self):
        if not self._adaptable:
            from venafi.tpp.attributes.adaptable_ca import AdaptableCAAttributes
    ...
    def msca(self):
        if not self._msca:
            from venafi.tpp.attributes.microsoft_ca import MicrosoftCAAttributes
    ...
    def self_signed(self):
        if not self._self_signed:
            from venafi.tpp.attributes.self_signed_ca import SelfSignedCAAttributes
...
class _ClientWork:
    ...
    def agent_connectivity(self):
        if not self._agent_connectivity:
            from venafi.tpp.attributes.client_agent_configuration_work import ClientAgentConfigurationWorkAttributes
    ...
    def agent_upgrade(self):
        if not self._agent_upgrade:
            from venafi.tpp.attributes.client_agent_automatic_upgrade_work import ClientAgentAutomaticUpgradeWorkAttributes
    ...
    def certificate_device_placement(self):
        if not self._certificate_device_placement:
            from venafi.tpp.attributes.server_agent_cert_device_placement_work import ServerAgentCertDevicePlacementWorkAttributes
    ...
    def certificate_discovery(self):
        if not self._certificate_discovery:
            from venafi.tpp.attributes.client_certificate_discovery_work import ClientCertificateDiscoveryWorkAttributes
    ...
    def certificate_enrollment_via_est_protocol(self):
        if not self._certificate_enrollment_via_est_protocol:
            from venafi.tpp.attributes.network_device_certificate_work import NetworkDeviceCertificateWorkAttributes
    ...
    def certificate_installation(self):
        if not self._certificate_installation:
            from venafi.tpp.attributes.certificate_provisioning_work import CertificateProvisioningWorkAttributes
    ...
    def dynamic_provisioning(self):
        if not self._dynamic_provisioning:
            from venafi.tpp.attributes.server_certificate_work import ServerCertificateWorkAttributes
    ...
    def ssh_device_placement(self):
        if not self._ssh_device_placement:
            from venafi.tpp.attributes.server_agent_ssh_device_placement_work import ServerAgentSSHDevicePlacementWorkAttributes
    ...
    def ssh_discovery(self):
        if not self._ssh_discovery:
            from venafi.tpp.attributes.client_agent_ssh_discovery_work import ClientAgentSSHDiscoveryWorkAttributes
    ...
    def ssh_key_usage(self):
        if not self._ssh_key_usage:
            from venafi.tpp.attributes.client_agent_ssh_key_usage_work import ClientAgentSSHKeyUsageWorkAttributes
    ...
    def user_certificate_creation(self):
        if not self._user_certificate_creation:
            from venafi.tpp.attributes.client_user_certificate_work import ClientUserCertificateWorkAttributes
...
class _Credential:
    ...
    def amazon(self):
        if not self._amazon:
            from venafi.tpp.attributes.amazon_credential import AmazonCredentialAttributes
    ...
    def certificate(self):
        if not self._certificate:
            from venafi.tpp.attributes.certificate_credential import CertificateCredentialAttributes
    ...
    def generic(self):
        if not self._generic:
            from venafi.tpp.attributes.generic_credential import GenericCredentialAttributes
    ...
    def password(self):
        if not self._password:
            from venafi.tpp.attributes.password_credential import PasswordCredentialAttributes
    ...
    def private_key(self):
        if not self._private_key:
            from venafi.tpp.attributes.private_key_credential import PrivateKeyCredentialAttributes
    ...
    def username_password(self):
        if not self._upcred:
            from venafi.tpp.attributes.username_password_credential import UsernamePasswordCredentialAttributes
...
class _Discovery:
    ...
    def network(self):
        if not self._network:
            from venafi.tpp.attributes.discovery import DiscoveryAttributes
...
class _Identity:
    ...
    def group(self):
        if not self._group:
            from venafi.tpp.attributes.group import GroupAttributes
    ...
    def user(self):
        if not self._user:
            from venafi.tpp.attributes.user import UserAttributes
...
class _Platforms:
    ...
    def auto_layout_manager(self):
        if not self._auto_layout_manager:
            from venafi.tpp.attributes.auto_layout_manager import AutoLayoutManagerAttributes
    ...
    def bulk_provisioning_manager(self):
        if not self._bulk_provisioning_manager:
            from venafi.tpp.attributes.bulk_provisioning_manager import BulkProvisioningManagerAttributes
    ...
    def ca_import_manager(self):
        if not self._ca_import_manager:
            from venafi.tpp.attributes.ca_import_manager import CAImportManagerAttributes
    ...
    def certificate_manager(self):
        if not self._certificate_manager:
            from venafi.tpp.attributes.certificate_manager import CertificateManagerAttributes
    ...
    def certificate_pre_enrollment(self):
        if not self._certificate_pre_enrollment:
            from venafi.tpp.attributes.certificate_pre_enrollment import CertificatePreEnrollmentAttributes
    ...
    def certificate_revocation(self):
        if not self._certificate_revocation:
            from venafi.tpp.attributes.certificate_revocation import CertificateRevocationAttributes
    ...
    def cloud_instance_monitor(self):
        if not self._cloud_instance_monitor:
            from venafi.tpp.attributes.cloud_instance_monitor import CloudInstanceMonitorAttributes
    ...
    def discovery_manager(self):
        if not self._discovery_manager:
            from venafi.tpp.attributes.discovery_manager import DiscoveryManagerAttributes
    ...
    def engines(self):
        if not self._engines:
            from venafi.tpp.attributes.venafi_platform import VenafiPlatformAttributes
    ...
    def monitor(self):
        if not self._monitor:
            from venafi.tpp.attributes.monitoring_module import MonitoringModuleAttributes
    ...
    def onboard_discovery_manager(self):
        if not self._onboard_discovery_manager:
            from venafi.tpp.attributes.onboard_discovery_manager import OnboardDiscoveryManagerAttributes
    ...
    def reporting(self):
        if not self._reporting:
            from venafi.tpp.attributes.reporter_service_module import ReporterServiceModuleAttributes
    ...
    def ssh_manager(self):
        if not self._ssh_manager:
            from venafi.tpp.attributes.ssh_manager import SSHManagerAttributes
    ...
    def validation_manager(self):
        if not self._validation_manager:
            from venafi.tpp.attributes.validation_manager import ValidationManagerAttributes
...
class _Workflow:
    ...
    def adaptable(self):
        if not self._adaptable:
            from venafi.tpp.attributes.adaptable_workflow import AdaptableWorkflowAttributes
    ...
    def standard(self):
        if not self._standard:
            from venafi.tpp.attributes.workflow import WorkflowAttributes
    ...
    def ticket(self):
        if not self._ticket:
            from venafi.tpp.attributes.workflow_ticket import WorkflowTicketAttributes
...
class _Attributes:
    ...
    def client_groups(self):
        if not self._client_groups:
            from venafi.tpp.attributes.client_group import ClientGroupAttributes
    ...
    def custom_fields(self):
        if not self._custom_fields:
            from venafi.tpp.attributes.metadata_list import MetadataListAttributes
            from venafi.tpp.attributes.metadata_text import MetadataTextAttributes
            from venafi.tpp.attributes.metadata_category import MetadataCategoryAttributes
            from venafi.tpp.attributes.metadata_choice import MetadataChoiceAttributes
            from venafi.tpp.attributes.metadata_datetime import MetadataDateTimeAttributes
            from venafi.tpp.attributes.metadata_identity import MetadataIdentityAttributes
            from venafi.tpp.attributes.metadata_base import IterableMeta
    ...
    def device(self):
        if not self._device:
            from venafi.tpp.attributes.device import DeviceAttributes
    ...
    def folder(self):
        if not self._folder:
            from venafi.tpp.attributes.policy import PolicyAttributes
    ...
    def placement_rules(self):
        if not self._placement_rules:
            from venafi.tpp.attributes.layout_rule_base import LayoutRuleBaseAttributes
